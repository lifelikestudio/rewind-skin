...
| Cart Drawer    | TINY, XXS, XS, SM, MD, MD-LG, LG, XL                           | Items (with products, quantity, subtotal footer), Empty (callout + CTA)                                                                                                               | Right-aligned side panel at LG/XL (456px) and MD-LG through SM (440px); centred near-full-width modal at XS and below. Header swaps logotype for title "Bag" + quantity badge. ProductCardCart extends search card with QuantitySelect and Remove. Solid white footer with subtotal section + multi-label checkout button. Empty state is content-height (not viewport-constrained). Product card internals stable across all breakpoints. All colours established — no new candidates. Interaction states (quantity +/−, remove link, card hover/press) not started — no frames provided. | — No candidates                                        | ✅ Declared visual scope extracted            |
